# Manuale utente — Gestionale Agostinelli

> Guida operativa per **Pietro** (proprietario / amministratore).
> Scritta in italiano, passo passo. Spiega *come* si usa il gestionale e *perché* le cose
> funzionano così (cosa succede ai conti e alla cassa).
>
> Aggiornato al 29 giugno 2026 · allineato al codice dell'applicazione.

---

## Come è organizzato questo manuale

Il manuale è diviso in due parti.

- **Parte I — Capire il gestionale.** Sette capitoli brevi che spiegano il "mentale" del
  sistema: competenza vs cassa, il modello a tre date, il conto economico, il cash flow, le
  Business Unit e il piano dei conti. Leggila una volta: ti farà capire tutto il resto.
- **Parte II — Capitoli operativi.** Un capitolo per ogni area dell'applicazione, con la guida
  passo-passo di ogni cosa che puoi creare o gestire.

Se hai fretta e vuoi solo "fare una cosa", salta alla Parte II. Ma i numeri nei report avranno
senso solo dopo aver letto la Parte I.

---

## Indice

### Parte I — Capire il gestionale
- [1. Benvenuto: a cosa serve e chi fa cosa](01-capire-il-gestionale.md#1-benvenuto)
- [2. Competenza vs cassa: il modello a tre date](01-capire-il-gestionale.md#2-il-modello-a-tre-date)
- [3. Il conto economico (il waterfall)](01-capire-il-gestionale.md#3-il-conto-economico)
- [4. Il cash flow (e perché è diverso dall'utile)](01-capire-il-gestionale.md#4-il-cash-flow)
- [5. Business Unit e piano dei conti](01-capire-il-gestionale.md#5-business-unit-e-piano-dei-conti)
- [6. Cosa NON è un costo o un ricavo](01-capire-il-gestionale.md#6-cosa-non-e-un-costo-o-un-ricavo)
- [7. Il ciclo di vita di un dato](01-capire-il-gestionale.md#7-il-ciclo-di-vita-di-un-dato)

### Parte II — Capitoli operativi

I capitoli sono raggruppati per area del menu. (I numeri di capitolo sono identificativi stabili e
non sempre consecutivi; i collegamenti "Precedente/Prossimo" in fondo a ogni capitolo ti guidano in
una lettura lineare di tutta la Parte II.)

**Operatività**
- [8. Dashboard](02-dashboard.md)
- [10. Eventi](04-eventi.md)
- [18. Scadenzario](13-scadenzario.md)

**Contabilità**
- [9. Movimenti](03-movimenti.md)
- [12. Import & smistamento](06-import-e-smistamento.md)
- [11. Spese ricorrenti](05-spese-ricorrenti.md)

**Analisi**
- [15. Report e Previsioni](09-reporting-e-previsioni.md)
- [21. Esporta dati](15-export.md)

**Configurazione**
- [14. Anagrafica](08-anagrafica.md)
- [20. Piano dei conti](14-piano-conti.md)
- [13. Regole di classificazione](07-keyword.md)
- [19. Situazione iniziale](12-situazione-iniziale.md)

**Riferimento**
- [16. Appendice — Cassa (funzione non attiva)](10-appendice-cassa.md)
- [17. Glossario finanziario](11-glossario.md)

---

## Le voci di menu (a colpo d'occhio)

Il menu a sinistra è raggruppato in quattro aree — *lavoro quotidiano → soldi → lettura → impostazioni*:

| Area | Voci |
|---|---|
| **Operatività** | Dashboard · Eventi · Scadenzario |
| **Contabilità** | Movimenti · Import & smistamento · Spese ricorrenti |
| **Analisi** | Report · Previsioni · Esporta dati |
| **Configurazione** | Anagrafica · Piano dei conti · Regole di classificazione · Situazione iniziale |

Tu sei **amministratore** e vedi tutte le voci. Un dipendente vede solo **Dashboard** ed
**Eventi**. (Vedi [capitolo 1](01-capire-il-gestionale.md#1-benvenuto).)

> La voce **Cassa** è oggi disattivata: ne parliamo nell'[appendice](10-appendice-cassa.md).
